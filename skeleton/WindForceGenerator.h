...
public:
	...
	void updateForce(physx::PxRigidDynamic* rigid, float duration) override {
		...
		Vector3D relativeVel = windVelocity - Vector3D(vel.x, vel.y, vel.z);
		Vector3D force = relativeVel * k1;
		if (k2 != 0.0f)
		{
			float speed = relativeVel.Modulo();
		...
		// aplicar la fuerza al cuerpo rígido
		rigid->addForce(physx::PxVec3(force.getX(), force.getY(), force.getZ()));
	}
